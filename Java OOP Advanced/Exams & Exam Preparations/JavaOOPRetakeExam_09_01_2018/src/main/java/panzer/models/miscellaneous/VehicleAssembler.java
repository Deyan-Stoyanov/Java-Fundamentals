...
public class VehicleAssembler implements Assembler {
    ...
    public long getTotalAttackModification() {
        return this.arsenalParts.stream().mapToInt(AttackModifyingPart::getAttackModifier).sum();
    }

    @Override
    public long getTotalDefenseModification() {
        return this.shellParts.stream().mapToInt(DefenseModifyingPart::getDefenseModifier).sum();
    }

    @Override
    public long getTotalHitPointModification() {
        return this.enduranceParts.stream().mapToInt(HitPointsModifyingPart::getHitPointsModifier).sum();
    }
